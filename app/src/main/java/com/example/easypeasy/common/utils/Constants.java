package com.example.easypeasy.common.utils;

public class Constants {
    public static final String API_KEY = "d26efdad5be347db9528c724054c4324";
    public static final String SPOONACULAR_BASE_URL = "https://api.spoonacular.com/";
    public static final String PAYLOAD_INSERT_INGREDIENT_FIELD = "payload_ingredient_field";
    public static final String PAYLOAD_INSERT_NUTRIENT_FIELD = "payload_nutrient_field";
    public static final String PAYLOAD_INSERT_INGREDIENT_FIELD_UNITS = "payload_units";
    public static final String REGEX_X = "x";
    public static final String ACTION_FETCH_RECIPES = "com.biotelemetry.stingray.ui.services.action.fetchrecipes";
}
